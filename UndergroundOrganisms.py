...
source_energy_drain = 0
infr_energy_drain = 0

ore_rand_max = 75
vein_size_max = 40
vein_size_min = 20
energy_cost_constant = -1
ore_gain = 5
...
class source(entity):
    ...
    def __init__(self, x, y, genes, energy):
    ...
    def behavior(self):
        ...
        if self.spawned == False: #initial deployment of feelers
            ...
            self.age += 1
        if self.affectResources(source_energy_drain, 0) == False:
            self.die()
    ...
    def spawn(self):
        ...
        next_grid[32][random_x] = source(random_x,32,passGenes(self.genes),starting_energy)
...
class infrastructure(entity):
    ...
    def affectResources(self, energy, depth):
        if depth < max_chain:
            return grid[self.connected_y][self.connected_x].affectResources(energy, depth + 1)
        return False
    ...
    def behavior(self):
        ...
        energy_change = infr_energy_drain
        if self.affectResources(energy_change, 0) == False:
            self.die()
...
class feeler(entity):
    ...
    def behavior(self):
        ...
        if self.state != 99:
            ...
            spawn = 1
            if not constructed:
                if self.affectResources(seed_energy_req // 2, 0) == True:
        ...
        if type(space) is block:
            if self.genes[self.state][direction][space.block_type] < 16:
                ...
                if construction_type == 3:
                    ...
                    energy_cost = ore_gain
                else:
                    energy_cost = energy_cost_constant
                if self.affectResources(energy_cost, 0) == True:
...
##                self.die()
##            elif move == 0:
        next_grid[self.y][self.x] = source(self.x, self.y, self.genes, starting_energy)
                #next_grid[self.y][self.x] = grid[self.y][self.x]
...
##                    grid[row][column] = block(column,row,2)
    ...
    for row in range(grid_size_y):
        for column in range(grid_size_x):
            ...
            if random_num == 0:
                grid[row][column] = block(column,row,2)
                vein_size = random.randint(vein_size_min, vein_size_max)
...
grid[32][grid_size_x // 5] = source(grid_size_x // 5,32,initialGenes(),starting_energy)
grid[5][grid_size_x * 2 // 5] = source(grid_size_x * 2 // 5,5,initialGenes(),starting_energy)
grid[40][grid_size_x * 3 // 5] = source(grid_size_x * 3 // 5,40,initialGenes(),starting_energy)
grid[50][grid_size_x * 4 // 5] = source(grid_size_x * 4 // 5,50,initialGenes(),starting_energy)
            ...
            if wait_round == True:
                if auto_restart == True:
                    ...
                    for n in range(spawn_new_total):
                        ...
                        grid[random_y][random_x] = source(random_x,random_y,initialGenes(),starting_energy)
                        alive += 1
