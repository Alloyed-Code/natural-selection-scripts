...
class tile ():
    ...
    def behavior(self):
        return
    def affectResources(self, metal, energy):
        return False
...
class source(entity):
    ...
    def behavior(self):
        ...
        #print(self.energy)
        #print(self.metal)
        if self.affectResources(source_metal_drain,source_energy_drain) == False:
...
class infrastructure(entity):
    ...
    def affectResources(self, metal, energy):
        return grid[self.connected_y][self.connected_x].affectResources(metal, energy)
    ...
    def behavior(self):
        ...
        if self.affectResources(metal_change, energy_change) == False:
            self.die()
...
class feeler(entity):
    ...
    def behavior(self):
        ...
        self.construct(self.x+1,self.y,1)
        self.construct(self.x,self.y+1,2)
        self.construct(self.x-1,self.y,3)
        ...
        if type(space) is block:
            if self.genes[self.state][direction][space.block_type] < 16:
                ...
                if construction_type == 3:
                    ...
                    energy_cost = construction_type +1
                if self.affectResources(metal_cost,energy_cost) == True:
...
def passGenes(genes):
    ...
    for gene in range(4): #direction
        for condition in range(cellMax + 1):
            genes[16][gene][condition] = mutation(genes[16][gene][condition], 16)
    return genes
